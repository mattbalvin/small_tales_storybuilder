/*
  # Small Tales Story Builder Database Schema

  1. New Tables
    - `users` - User accounts and profiles with role-based access
    - `stories` - Story metadata and settings (publicly accessible)
    - `story_pages` - Individual story pages with content and interactions
    - `media_assets` - Uploaded files and media with user ownership
    - `analytics_events` - User interaction tracking and analytics

  2. Security
    - Enable RLS on all tables
    - Users can manage their own profiles and media
    - Stories and pages are publicly readable but require authentication to modify
    - Analytics events can be inserted by anyone, read by story authors

  3. Performance
    - Comprehensive indexing for common queries
    - Automatic updated_at timestamp triggers
    - Optimized for story reading and authoring workflows
*/
