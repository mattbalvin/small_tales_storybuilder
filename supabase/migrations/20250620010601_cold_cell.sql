/*
  ...
  2. Security
    ...
    - Add policies for collaboration access control
    - Update existing story and story_pages policies

  3. Functions
    - `get_current_user_id()` - Helper function for auth.uid()
    - `add_story_owner()` - Trigger function to auto-add story owner
    - `user_has_story_access()` - Permission checking function

  4. Changes
    - Updates all story-related policies to use collaboration system
    - Backfills existing stories with owner collaborator records (only for valid authors)
*/
...
RETURNS TRIGGER AS $$
BEGIN
  -- Only insert if author_id is not null
...
-- Only insert for stories that have a valid author_id and where the user exists
INSERT INTO story_collaborators (story_id, user_id, permission_level, invited_by, accepted_at)
SELECT s.id, s.author_id, 'owner', s.author_id, now()
FROM stories s
INNER JOIN users u ON s.author_id = u.id  -- Ensure the author exists in users table
WHERE s.author_id IS NOT NULL  -- Ensure author_id is not null
  AND NOT EXISTS (
    SELECT 1 FROM story_collaborators sc
    WHERE sc.story_id = s.id AND sc.user_id = s.author_id
  );