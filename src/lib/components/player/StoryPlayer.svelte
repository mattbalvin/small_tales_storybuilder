<script lang="ts">
  ...
  function playPageAudio() {
    ...
    const narratedElements = currentPage.content.elements
    ...
    if (narratedElements.length > 0) {
      ...
      if (currentPage?.content?.audioElements) {
        ...
        if (narrationAudio?.properties?.src) {
          ...
          // No audio, auto-advance after reading time if not in preview mode
          if (!isPreviewMode) {
  ...
  function playElementNarration(element: any) {
    ...
    try {
      ...
      currentNarrationPlayer = new NarrationPlayer(element.properties.narrationData);
      ...
      currentNarrationPlayer.onPlaybackEnd = () => {
        // When narration ends, clear state
        currentNarrationPlayer = null;
        activeNarrationElementId = null;
        currentWord = null;
        highlightedWordIndex = -1;
        
        // If not in preview mode, advance to next page
        if (isAutoReading && !isPreviewMode) {
          nextPage();
        }
      };
      ...
      // If not in preview mode, advance to next page after a delay
      if (isAutoReading && !isPreviewMode) {
  ...
  function playAudio(src: string) {
    ...
    audioElement.addEventListener('error', () => {
      ...
      // Auto-advance after estimated reading time if audio fails and not in preview mode
      if (isAutoReading && !isPreviewMode) {
        const readingTime = estimateReadingTime()
        setTimeout(() => {
          nextPage()
  ...
  function renderTextWithHighlight(text: string, highlightWord: string | null, highlightColor: string = '#F0B464', enableGlow: boolean = true) {
    if (!text || !highlightWord) return text;
    
    // Create a safe regex pattern from the highlight word
    ...
    // Create the HTML with highlighted word
    let result = '';
    let i = 0;
    ...
    text.replace(regex, (match) => {
      ...
      const glowStyle = enableGlow ? `box-shadow: 0 0 5px ${highlightColor};` : '';
      result += `<span class="word-highlight" style="background-color: ${highlightColor}40; color: inherit; font-weight: bold; transform: scale(1.05); display: inline-block; border-radius: 4px; padding: 0 2px; ${glowStyle}">${match}</span>`;
                ...
                >
                  {#if element.type === 'text' && showText}
                    <div
                      ...
                      "
                    >
                      {#if activeNarrationElementId === element.id && currentWord}
                        {@html renderTextWithHighlight(
                          element.properties?.text || '', 
                          currentWord,
                          element.properties?.narrationHighlightColor || '#F0B464',
                          element.properties?.narrationHighlightGlow !== false
                        )}
                      {:else}
                        {element.properties?.text || ''}
                      {/if}
